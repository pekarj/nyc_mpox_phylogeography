...
# set variables based on main.ipynb
κ_msm=0.77
α_msm=0.10  # 2000–2020
κ_hem=1.68
α_hem=0.01 # 2000–2010
κ_hew=2.14
α_hew=0.01 # 2010
inf_period=28
...
### Outputs
if !isdefined(@__MODULE__,:R0only) R0only = true end
if !isdefined(@__MODULE__,:R0finite) R0finite = false end
if !isdefined(@__MODULE__,:R0deplete) R0deplete = true end
if !isdefined(@__MODULE__,:R0assortative) R0assortative = true end

# Define the array of population sizes
# pop_sizes = [100000]
popsize = 235000
...
hline!([1],linestyle=:dot,color=:black,label="")|>display
    ...
    elseif any(R0deplete)
        if any(R0deplete) # generate new: takes ~ 1 hour
            ...
            pop=copy(deg)
            num_sims = 1000
            Rmat=Matrix{eltype(deg)}(undef, popsize, num_sims) # initialize result vector
            # deg_mat = Matrix{eltype(deg)}(undef, popsize, num_sims) # initialize result vector
            deg_mat = DataFrame(Matrix{Float64}(undef, popsize, num_sims), :auto)
            ...
            inf_period_string = string(inf_period)

            filename_r0 = "output/R0sdeplete" * verstring * "k." * inf_period_string * "d.csv"
            CSV.write(filename_r0, R0deplete_output)
            
            deg_df = DataFrame(deg_column = deg) # Convert vector to DataFrame with a column name
            filename_deg = "output/deg" * verstring * "k." * inf_period_string * "d.csv"
            CSV.write(filename_deg, deg_df)

            # CSV.write("output/degmat"*verstring*"k.csv", DataFrame(deg_mat))
            filename_degmat = "output/degmat" * verstring * "k." * inf_period_string * "d.csv"
            CSV.write(filename_degmat, DataFrame(deg_mat))
        ...
        p = plot(sizerange, qlines[:,1].*[0.1 0.5], ribbon=(qlines[:,2].*[0.1 0.5], qlines[:,3].*[0.1 0.5]),
        ...
        hline!(p, [1], linestyle=:dot, color=:black, label="")
        savefig(p, "./output/R_over_sexual_network_popsize_$(popsize).$(inf_period)d.png")
        # end
...
display(loglogplot)
# save the plot
savefig(loglogplot, "./output/degree_distribution.png")
